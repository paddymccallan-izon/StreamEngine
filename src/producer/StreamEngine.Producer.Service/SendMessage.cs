...
namespace StreamEngine.Producer.Service
{
    public class SendMessage : ISendMessage
    {
            ...
            ILogger<SendMessage> logger)
        {
            ...
            userName = this.configuration.GetSection("CREDENTIALS").GetSection("USER_NAME").Value;
            password = this.configuration.GetSection("CREDENTIALS").GetSection("PASSWORD").Value;
            topic = this.configuration.GetSection("TOPICS").GetSection("TEST_TOPIC").Value;
        }
                    ...
                    }, (deliveryReport) =>
                    {
                        ...
                        else
                        {
                            logger.LogInformation(message: $"Produced message to: {deliveryReport.TopicPartitionOffset} " +
                                $"\n Delivery timestamp: {deliveryReport.Timestamp}, {DateTime.UtcNow.ToString()}");
                        }
                    });
                }
                catch (Exception ex)
